# rough_env_cfg.py
#
# Rough terrain configuration for G1.
...
class G1Rewards(RewardsCfg):
    """Standard reward terms for G1 locomotion."""
    ...
    feet_air_time = RewTerm(func=mdp.feet_air_time_positive_biped, weight=0.25, params={"command_name": "base_velocity", "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_ankle_roll_link"), "threshold": 0.4})
    ...
    joint_deviation_arms = RewTerm(func=mdp.joint_deviation_l1, weight=-0.1, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_shoulder_.*", ".*_elbow_.*"])})
...
class G1RoughEnvCfg_PLAY(G1RoughEnvCfg):
    def __post_init__(self):
        ...
        # Play / Evaluation settings
        self.scene.num_envs = 50
        self.scene.env_spacing = 2.5
        self.episode_length_s = 40.0
        
        # Terrain adjustment for play (fixed grid, no curriculum)
